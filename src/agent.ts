...
export async function* runAgentStreaming(
  ...
  log.info(`Starting streaming with ${messages.length} messages, ${customTools.length} custom tools`);
  ...
  while (true) {
    ...
    for await (const event of s) {
      if (event.type === "text_delta") {
        ...
        yield { type: "text_delta", delta: event.delta, accumulated: fullText };
      } else if (event.type === "toolcall_end") {
        toolCalls.push(event.toolCall);
        log.info(`Tool call: ${event.toolCall.name}`);
    ...
    if (toolCalls.length === 0) {
      log.info(`No tool calls, returning response (${fullText.length} chars)`);
